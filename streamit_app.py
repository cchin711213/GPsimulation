...
# FIX: Changed fr" " to r" " here because there are no variables in the title.
# This prevents Python from thinking {Cs} is a variable.
st.title(r"$\text{Cs}^{133}$ Bose-Einstein Condensate Explorer")
...
if st.button("Run Simulation"):
    with st.spinner("Calculating Ground State..."):
        ...
        mu = np.real(np.sum(ke_dens + V*np.abs(psi)**2 + g1d*np.abs(psi)**4) * dx) / N_tot
        mu_hz = (mu * (E_scale / ms)) / (2 * np.pi * hbar_si)

        # Plotting
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.plot(x, np.abs(psi)**2, color='royalblue', lw=2, label='Density')
        ax.fill_between(x, np.abs(psi)**2, color='royalblue', alpha=0.2)
        
        # FIX: Used fr" " with double braces {{ }} for LaTeX and single { } for variables
        ax.set_title(fr"Ground State Profile ($\mu = {mu_hz:.2f}$ Hz)")
        ax.set_xlabel(r"Position ($\mu$m)")
        ax.set_ylabel(r"Density (atoms/$\mu$m)")
        ax.grid(True, alpha=0.3)
        st.pyplot(fig)
else:
    st.info("Adjust the sliders and click 'Run Simulation' to begin.")
